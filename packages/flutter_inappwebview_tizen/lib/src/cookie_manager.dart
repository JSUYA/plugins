...
class TizenCookieManagerCreationParams
  ...
  const TizenCookieManagerCreationParams({this.webViewEnvironment});

  /// Creates a [TizenCookieManagerCreationParams] instance based on [PlatformCookieManagerCreationParams].
  factory TizenCookieManagerCreationParams.fromPlatformCookieManagerCreationParams(
    // Recommended placeholder to prevent being broken by platform interface.
    // ignore: avoid_unused_constructor_parameters
    PlatformCookieManagerCreationParams params,
  ) {
    return TizenCookieManagerCreationParams(
...
class TizenCookieManager extends PlatformCookieManager with ChannelController {
  ...
  ///Gets the [TizenCookieManager] shared instance.
  static TizenCookieManager instance({
    TizenWebViewEnvironment? webViewEnvironment,
  }) {
    if (webViewEnvironment == null) {
      ...
      return TizenCookieManager(
        TizenCookieManagerCreationParams(
  ...
  static TizenCookieManager _init() {
    _instance = TizenCookieManager(TizenCookieManagerCreationParams());
    return _instance!;
  }
  ...
  static PlatformInAppWebViewController _requireController(
    ...
    if (controller == null) {
      throw UnsupportedError(
        'This cookie operation requires a webViewController on flutter_inappwebview_tizen.',
      );
    }
    return controller;
  }
  ...
  Future<PlatformInAppWebViewController> _requireJavascriptFallbackController({
    ...
    if (rejectHttpOnly) {
      throw UnsupportedError(
        'JavaScript cookie fallback does not support HttpOnly cookies on flutter_inappwebview_tizen.',
      );
    }
    ...
    if (currentUrl == null || !_isSameOrigin(currentUrl, url)) {
      throw UnsupportedError(
        'JavaScript cookie fallback only supports same-origin cookies for the currently loaded page on flutter_inappwebview_tizen.',
      );
    }
