...
class WebView : public PlatformView {
 public:
  ...
  static void ClearAllCacheAll();
  static void DestroyRetiredNativeViews();
 ...
 private:
  struct RetiredNativeView {
    Evas_Object* webview_instance = nullptr;
    Ecore_Evas* ecore_evas = nullptr;
  };
  ...
  void DetachInputFocus();
  static RetiredNativeView TakeRetiredNativeView();
  static void StoreRetiredNativeView(Evas_Object* webview_instance,
  ...
  flutter::EncodableMap settings_;
  Ewk_Mouse_Button_Type mouse_button_type_ = (Ewk_Mouse_Button_Type)0;
  bool ime_attached_ = false;

  static std::set<WebView*> instances_;
  static std::mutex instances_mutex_;
  static std::vector<RetiredNativeView> retired_native_views_;
  static std::mutex retired_native_views_mutex_;
};
