...
///Controls a WebView, such as an [InAppWebView] widget instance, a [LweHeadlessInAppWebView] instance or [LweInAppBrowser] WebView instance.
///
...
///callback. Instead, if you are using an [LweInAppBrowser] instance, you can get it through the [LweInAppBrowser.webViewController] attribute.
class LweInAppWebViewController extends PlatformInAppWebViewController
  ...
  Set<LweWebMessageChannel> _webMessageChannels = Set();
  Set<LweWebMessageListener> _webMessageListeners = Set();
  ...
  @override
  late LweWebStorage webStorage;

  LweInAppWebViewController(PlatformInAppWebViewControllerCreationParams params)
    : super.implementation(
        params is LweInAppWebViewControllerCreationParams
            ? params
            : LweInAppWebViewControllerCreationParams.fromPlatformInAppWebViewControllerCreationParams(
                params,
              ),
      ) {
  ...
  static final LweInAppWebViewController _staticValue =
      LweInAppWebViewController(
        LweInAppWebViewControllerCreationParams(id: null),
      );

  factory LweInAppWebViewController.static() {
    return _staticValue;
  }

  LweInAppWebViewController.fromInAppBrowser(
    PlatformInAppWebViewControllerCreationParams params,
    MethodChannel channel,
    LweInAppBrowser inAppBrowser,
  ...
  void _init(PlatformInAppWebViewControllerCreationParams params) {
    ...
    webStorage = LweWebStorage(
      LweWebStorageCreationParams(
        localStorage: LweLocalStorage.defaultStorage(controller: this),
        sessionStorage: LweSessionStorage.defaultStorage(controller: this),
      ),
    );
    ...
    if (params.webviewParams is PlatformInAppWebViewWidgetCreationParams) {
      ...
      if (keepAlive != null) {
        ...
        if (props == null) {
          ...
          _webMessageChannels =
              props.webMessageChannels as Set<LweWebMessageChannel>;
          _webMessageListeners =
              props.webMessageListeners as Set<LweWebMessageListener>;
  ...
  bool _isMissingOrUnsupported(dynamic error) {
    if (error is MissingPluginException) {
      return true;
    }
    if (error is PlatformException) {
      final code = error.code.toLowerCase();
      return code == 'unimplemented' || code == 'unsupported operation';
    }
    return false;
  }

  Future<T?> _invokeMethodWithFallback<T>(
    String method,
    ...
    try {
      ...
      if (fallback != null && _isMissingOrUnsupported(error)) {
        return await fallback();
      }
      rethrow;
    }
  ...
  String _registerTemporaryJavaScriptHandler(
    String prefix,
    void Function(List<dynamic> args) onCall,
  ) {
    final handlerName =
        '__flutter_inappwebview_${prefix}_${id}_${DateTime.now().microsecondsSinceEpoch}';
    _javaScriptHandlersMap[handlerName] = (args) async {
      try {
        onCall(args);
      } finally {
        _javaScriptHandlersMap.remove(handlerName);
      }
      return null;
    };
    return handlerName;
  ...
  Future<void> _injectJavascriptFileFromUrlWithJavascript({
    required WebUri urlFile,
    ScriptHtmlTagAttributes? scriptHtmlTagAttributes,
  }) async {
    String? onLoadHandlerName;
    String? onErrorHandlerName;
    if (scriptHtmlTagAttributes?.onLoad != null) {
      onLoadHandlerName = _registerTemporaryJavaScriptHandler(
        'script_load',
        (_) => scriptHtmlTagAttributes!.onLoad!(),
      );
    }
    if (scriptHtmlTagAttributes?.onError != null) {
      onErrorHandlerName = _registerTemporaryJavaScriptHandler(
        'script_error',
        (_) => scriptHtmlTagAttributes!.onError!(),
      );
    }
...
(function() {
  ...
  script.src = ${jsonEncode(urlFile.toString())};
  script.type = ${jsonEncode(scriptHtmlTagAttributes?.type ?? 'text/javascript')};
  if (${jsonEncode(scriptHtmlTagAttributes?.id)} != null) {
    script.id = ${jsonEncode(scriptHtmlTagAttributes?.id)};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.async)} != null) {
    script.async = ${scriptHtmlTagAttributes?.async == true};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.defer)} != null) {
    script.defer = ${scriptHtmlTagAttributes?.defer == true};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.noModule)} != null) {
    script.noModule = ${scriptHtmlTagAttributes?.noModule == true};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.crossOrigin?.toValue())} != null) {
    script.crossOrigin = ${jsonEncode(scriptHtmlTagAttributes?.crossOrigin?.toValue())};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.integrity)} != null) {
    script.integrity = ${jsonEncode(scriptHtmlTagAttributes?.integrity)};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.nonce)} != null) {
    script.nonce = ${jsonEncode(scriptHtmlTagAttributes?.nonce)};
  }
  if (${jsonEncode(scriptHtmlTagAttributes?.referrerPolicy?.toValue())} != null) {
    script.referrerPolicy = ${jsonEncode(scriptHtmlTagAttributes?.referrerPolicy?.toValue())};
  }
  if (${jsonEncode(onLoadHandlerName)} != null) {
    script.onload = function() {
      if (window.flutter_inappwebview &&
          window.flutter_inappwebview.callHandler) {
        window.flutter_inappwebview.callHandler(${jsonEncode(onLoadHandlerName)});
      }
    };
  }
  if (${jsonEncode(onErrorHandlerName)} != null) {
    script.onerror = function() {
      if (window.flutter_inappwebview &&
          window.flutter_inappwebview.callHandler) {
        window.flutter_inappwebview.callHandler(${jsonEncode(onErrorHandlerName)});
      }
    };
  }
  parent.appendChild(script);
      ...
      if (onLoadHandlerName != null) {
        _javaScriptHandlersMap.remove(onLoadHandlerName);
      }
      if (onErrorHandlerName != null) {
        _javaScriptHandlersMap.remove(onErrorHandlerName);
      }
      rethrow;
    }
  }

  Future<void> _injectCSSCodeWithJavascript({required String source}) async {
    await evaluateJavascript(
      source:
          '''
(function() {
  ...
  style.type = 'text/css';
  style.appendChild(document.createTextNode(${jsonEncode(source)}));
  ...
  Future<void> _injectCSSFileFromUrlWithJavascript({
    required WebUri urlFile,
...
(function() {
  ...
  link.rel = ${jsonEncode(cssLinkHtmlTagAttributes?.alternate == true ? 'alternate stylesheet' : 'stylesheet')};
  link.href = ${jsonEncode(urlFile.toString())};
  if (${jsonEncode(cssLinkHtmlTagAttributes?.id)} != null) {
    link.id = ${jsonEncode(cssLinkHtmlTagAttributes?.id)};
  }
  if (${jsonEncode(cssLinkHtmlTagAttributes?.crossOrigin?.toValue())} != null) {
    link.crossOrigin = ${jsonEncode(cssLinkHtmlTagAttributes?.crossOrigin?.toValue())};
  }
  if (${jsonEncode(cssLinkHtmlTagAttributes?.disabled)} != null) {
    link.disabled = ${cssLinkHtmlTagAttributes?.disabled == true};
  }
  if (${jsonEncode(cssLinkHtmlTagAttributes?.integrity)} != null) {
    link.integrity = ${jsonEncode(cssLinkHtmlTagAttributes?.integrity)};
  }
  if (${jsonEncode(cssLinkHtmlTagAttributes?.media)} != null) {
    link.media = ${jsonEncode(cssLinkHtmlTagAttributes?.media)};
  }
  if (${jsonEncode(cssLinkHtmlTagAttributes?.referrerPolicy?.toValue())} != null) {
    link.referrerPolicy = ${jsonEncode(cssLinkHtmlTagAttributes?.referrerPolicy?.toValue())};
  }
  if (${jsonEncode(cssLinkHtmlTagAttributes?.title)} != null) {
    link.title = ${jsonEncode(cssLinkHtmlTagAttributes?.title)};
  }
  ...
  Future<String?> _getSelectedTextWithJavascript() async {
    final selectedText = await evaluateJavascript(
      source: '''
(function() {
  ...
  Future<CallAsyncJavaScriptResult?> _callAsyncJavaScriptWithJavascript({
    required String functionBody,
    Map<String, dynamic> arguments = const <String, dynamic>{},
  }) async {
    final String resultKey =
        '__flutter_inappwebview_async_result_${id}_${DateTime.now().microsecondsSinceEpoch}';
    ...
    try {
      await evaluateJavascript(
        source:
            '''
(function() {
  var flutterInAppWebViewArgs = ${jsonEncode(arguments)};
  var resultKey = ${jsonEncode(resultKey)};
  window[resultKey] = { status: 'pending' };
  Promise.resolve((async function(args) {
$functionBody
  })(flutterInAppWebViewArgs)).then(function(value) {
    window[resultKey] = { status: 'success', value: value };
  }).catch(function(error) {
    var message = error != null
        ? (error.message || error.toString())
        : 'Unknown error';
    window[resultKey] = { status: 'error', error: message };
  });
})();
        ''',
      );
    } catch (error) {
      return CallAsyncJavaScriptResult(error: error.toString());
    }

    final DateTime timeoutAt = DateTime.now().add(const Duration(seconds: 10));
    while (DateTime.now().isBefore(timeoutAt)) {
      final dynamic result = await evaluateJavascript(
        source:
            '''
(function() {
  var resultKey = ${jsonEncode(resultKey)};
  var result = window[resultKey];
  ...
  return 'error::' + JSON.stringify(result.error);
      ...
      if (result != null) {
        ...
        if (resultString.startsWith('success::')) {
          return CallAsyncJavaScriptResult(
            value: json.decode(resultString.substring('success::'.length)),
          );
        }
        return CallAsyncJavaScriptResult(
          error:
              json
                  .decode(resultString.substring('error::'.length))
                  ?.toString() ??
              'Unknown error',
        );
      }
    ...
    await evaluateJavascript(
      source: 'delete window[${jsonEncode(resultKey)}];',
    );
    return CallAsyncJavaScriptResult(
      error: 'Timed out waiting for callAsyncJavaScript result.',
    );
  }

  Future<bool> _canScrollVerticallyWithJavascript() async {
    return await evaluateJavascript(
          source: '''
(function() {
  var initialY = window.pageYOffset || window.scrollY || 0;
  window.scrollTo(0, initialY + 1);
  var scrolledY = window.pageYOffset || window.scrollY || 0;
  ...
  var clientHeight = Math.max(doc ? doc.clientHeight : 0, body ? body.clientHeight : 0, window.innerHeight || 0);
  ...
  Future<bool> _canScrollHorizontallyWithJavascript() async {
    return await evaluateJavascript(
          source: '''
(function() {
  var initialX = window.pageXOffset || window.scrollX || 0;
  window.scrollTo(initialX + 1, window.pageYOffset || window.scrollY || 0);
  var scrolledX = window.pageXOffset || window.scrollX || 0;
  window.scrollTo(initialX, window.pageYOffset || window.scrollY || 0);
  if (scrolledX > initialX) {
    return true;
  }
  var doc = document.documentElement;
  var body = document.body;
  var scrollWidth = Math.max(doc ? doc.scrollWidth : 0, body ? body.scrollWidth : 0);
  var clientWidth = Math.max(doc ? doc.clientWidth : 0, body ? body.clientWidth : 0, window.innerWidth || 0);
  ...
  Future<bool> _isSecureContextWithJavascript() async {
    return await evaluateJavascript(
          source: 'window.isSecureContext === true;',
        ) ==
        true;
  }

  Future<void> _postWebMessageWithJavascript({
    required WebMessage message,
    WebUri? targetOrigin,
  }) async {
    if ((message.ports?.isNotEmpty ?? false)) {
      throw UnsupportedError(
        'postWebMessage with transferable ports is not implemented on flutter_inappwebview_lwe.',
      );
    }

    final resolvedTargetOrigin = targetOrigin?.toString().isNotEmpty == true
        ? targetOrigin.toString()
        : '*';
    final payload = message.type == WebMessageType.ARRAY_BUFFER
        ? 'new Uint8Array(${jsonEncode((message.data as Uint8List).toList())}).buffer'
        : jsonEncode(message.data);
    await evaluateJavascript(
      source:
          'window.postMessage($payload, ${jsonEncode(resolvedTargetOrigin)});',
    );
  }

  Future<void> _pauseAllMediaPlaybackWithJavascript() async {
    await evaluateJavascript(
      source: '''
Array.prototype.forEach.call(
  ...
  Future<void> _closeAllMediaPresentationsWithJavascript() async {
    await evaluateJavascript(
      source: '''
(function() {
  Array.prototype.forEach.call(
  ...
  Future<MediaPlaybackState?> _requestMediaPlaybackStateWithJavascript() async {
    final state = await evaluateJavascript(
      source: '''
(function() {
  ...
  return 2;
    ...
    return MediaPlaybackState.fromNativeValue((state as num?)?.toInt());
  }

  Future<bool> _isInFullscreenWithJavascript() async {
    return await evaluateJavascript(
          source: '''
(function() {
  ...
  WebResourceError? _parseWebResourceError(Map<String, dynamic>? errorMap) {
    ...
    final WebResourceErrorType type =
        WebResourceErrorType.fromNativeValue(nativeType is int ? nativeType : null) ??
    ...
    switch (call.method) {
      ...
      case "onPrintRequest":
        if ((webviewParams != null &&
          ...
          WebUri? uri = url != null ? WebUri(url) : null;
          LwePrintJobController? printJob = printJobId != null
              ? LwePrintJobController(
                  LwePrintJobControllerCreationParams(id: printJobId),
                )
              : null;
...
""",
  ...
  Future<void> injectJavascriptFileFromUrl({
    ...
    await _invokeMethodWithFallback<void>(
      'injectJavascriptFileFromUrl',
      args,
      fallback: () async {
        await _injectJavascriptFileFromUrlWithJavascript(
          urlFile: urlFile,
          scriptHtmlTagAttributes: scriptHtmlTagAttributes,
        );
      },
  ...
  Future<void> injectCSSCode({required String source}) async {
    ...
    await _invokeMethodWithFallback<void>(
      ...
      args,
      fallback: () async {
        await _injectCSSCodeWithJavascript(source: source);
      },
    );
  }
  ...
  Future<void> injectCSSFileFromUrl({
    ...
    await _invokeMethodWithFallback<void>(
      ...
      args,
      fallback: () async {
        await _injectCSSFileFromUrlWithJavascript(
          urlFile: urlFile,
          cssLinkHtmlTagAttributes: cssLinkHtmlTagAttributes,
        );
      },
  ...
  @override
  Future<LwePrintJobController?> printCurrentPage({
  ...
  Future<String?> getSelectedText() async {
    Map<String, dynamic> args = <String, dynamic>{};
    return await _invokeMethodWithFallback<String?>(
      'getSelectedText',
      args,
      fallback: _getSelectedTextWithJavascript,
    );
  }
...
(function() {
  ...
  Future<CallAsyncJavaScriptResult?> callAsyncJavaScript({
    ...
    if (contentWorld != null && contentWorld.name != ContentWorld.PAGE.name) {
      throw UnsupportedError(
        'callAsyncJavaScript only supports ContentWorld.PAGE on flutter_inappwebview_lwe.',
      );
    }
    Map<String, dynamic> args = <String, dynamic>{};
    ...
    args.putIfAbsent('contentWorld', () => contentWorld?.toMap());
    var data = await _invokeMethodWithFallback<dynamic>(
      'callAsyncJavaScript',
      args,
      fallback: () => _callAsyncJavaScriptWithJavascript(
        functionBody: functionBody,
        arguments: arguments,
      ),
    );
    if (data == null || data is CallAsyncJavaScriptResult) {
      return data as CallAsyncJavaScriptResult?;
    }
    data = json.decode(data);
    return CallAsyncJavaScriptResult(
      value: data["value"],
  ...
  Future<bool> isSecureContext() async {
    Map<String, dynamic> args = <String, dynamic>{};
    return await _invokeMethodWithFallback<bool>(
          'isSecureContext',
          args,
          fallback: _isSecureContextWithJavascript,
        ) ??
        false;
  }

  @override
  Future<LweWebMessageChannel?> createWebMessageChannel() async {
    Map<String, dynamic> args = <String, dynamic>{};
    Map<String, dynamic>? result = (await channel?.invokeMethod(
      'createWebMessageChannel',
      args,
    ))?.cast<String, dynamic>();
    final webMessageChannel = LweWebMessageChannel.static().fromMap(result);
  ...
  Future<void> postWebMessage({
    ...
    await _invokeMethodWithFallback<void>(
      ...
      args,
      fallback: () async {
        await _postWebMessageWithJavascript(
          message: message,
  ...
  Future<void> addWebMessageListener(
    ...
    _webMessageListeners.add(webMessageListener as LweWebMessageListener);
  ...
  Future<bool> canScrollVertically() async {
    ...
    return await _invokeMethodWithFallback<bool>(
          'canScrollVertically',
          args,
          fallback: _canScrollVerticallyWithJavascript,
        ) ??
        false;
  }
  ...
  Future<bool> canScrollHorizontally() async {
    ...
    return await _invokeMethodWithFallback<bool>(
          ...
          args,
          fallback: _canScrollHorizontallyWithJavascript,
        ) ??
        false;
  }
  ...
  Future<void> pauseAllMediaPlayback() async {
    ...
    await _invokeMethodWithFallback<void>(
      ...
      args,
      fallback: () async {
        await _pauseAllMediaPlaybackWithJavascript();
      },
    );
  }
  ...
  Future<void> closeAllMediaPresentations() async {
    ...
    await _invokeMethodWithFallback<void>(
      ...
      args,
      fallback: () async {
        await _closeAllMediaPresentationsWithJavascript();
      },
    );
  }
  ...
  Future<MediaPlaybackState?> requestMediaPlaybackState() async {
    Map<String, dynamic> args = <String, dynamic>{};
    final state = await _invokeMethodWithFallback<dynamic>(
      'requestMediaPlaybackState',
      args,
      fallback: _requestMediaPlaybackStateWithJavascript,
    );
    if (state is MediaPlaybackState) {
      return state;
    }
    return MediaPlaybackState.fromNativeValue((state as num?)?.toInt());
  }
  ...
  Future<bool> isInFullscreen() async {
    ...
    return await _invokeMethodWithFallback<bool>(
          'isInFullscreen',
          args,
          fallback: _isInFullscreenWithJavascript,
        ) ??
        false;
  }
...
extension InternalInAppWebViewController on LweInAppWebViewController {
