...
// BluetoothManager owns all Tizen Bluetooth state used by the plugin:
// adapter lifecycle, scan state, the per-device GATT clients and the in-flight
// request bookkeeping. It is the only place that talks to the bt_* C API.
//
// All public entry points (`HandleMethodCall`) and all Emit* helpers must be
// called from the platform main thread. Native bt_* callbacks fire on worker
// threads and re-enter through `DispatchToMain()` before touching any state.
class BluetoothManager {
 ...
 private:
  ...
  // ----- Event emit helpers (wrap EventDispatcher::EmitEvent) ------------
  ...
  // ----- Main-thread handlers fed by bt_* callbacks ----------------------
  ...
  // ----- C callbacks fed back to the manager via DispatchToMain ----------
