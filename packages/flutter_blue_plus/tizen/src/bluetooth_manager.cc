...
// Replaces all entries of `target` belonging to `remote_id` with `incoming`.
...
void BluetoothManager::EnsureInitialized() {
  ...
  if (adapter_state_ == kBmAdapterUnavailable) {
    supported_ = false;
    unsupported_reason_ = "Bluetooth is not supported on this device.";
  }
}
...
void BluetoothManager::OnGattRequestCompletedOnMain(PendingGattRequest info,
  ...
  // If the request is no longer pending, it was already failed by
  // FailPendingRequestsForRemoteId() (disconnect or service reset) and the
  // handle may have been destroyed; drop this late completion.
  if (pending_gatt_requests_.erase(info.handle_key) == 0) {
    return;
  }
