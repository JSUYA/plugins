...
- `vpn` is not reported separately. Proxy or otherwise unknown network types are reported as `other`.
- `satellite` is not reported separately because the Tizen network API does not expose it.
