...
*/
...

  {
    ...
    evas_object_key_grab(btn, "Down", 0, 0, EINA_TRUE);
    evas_event_feed_key_down(
        evas_object_evas_get(btn), "Down", "Down", "Down", "Down",
...

  {
    ...
    LOG_ERROR("CJS TIZEN WINDOW PRESSED!!!!!!!!!!! ");
    elm_object_text_set(btn2, "333");
  }
*/
  /*
    static void btn_mouse_down_cb(void *data, Evas *e, void *event_info) {
      LOG_ERROR("CJS CHECK");
    }
    static void btn_mouse_up_cb(void *data, Evas *e, void *event_info) {
      LOG_ERROR("CJS CHECK");
    }

    static void win_p_mouse_down_cb(void *data, Evas *e, Evas_Object *obj,
                                    void *event_info) {
      LOG_ERROR("CJS CHECK");
    }
    static void win_mouse_down_cb(void *data, Evas *e, void *event_info) {
      LOG_ERROR("CJS CHECK");
    }
    static void win_mouse_up_cb(void *data, Evas *e, void *event_info) {
      LOG_ERROR("CJS CHECK");
    }*/
  ...
  Evas_Object *contents(Evas_Object *platformview_window_) {
    ...
    evas_object_smart_callback_add(button, "clicked", _btn_cb,
                                   platformview_window_);
    ...
    s_info.win = platformview_window_;
  ...
  bool OnCreate() {
    ...
    /*
        ...
        evas_event_callback_add(evas_object_evas_get(window_),
       EVAS_CALLBACK_KEY_UP, key_up_cb, this);
    ...
    elm_win_type_set(surface_win, ELM_WIN_BASIC);
    elm_win_focus_highlight_enabled_set(surface_win, EINA_TRUE);
    elm_win_autodel_set(surface_win, EINA_TRUE);
    // elm_win_alpha_set(surface_win, EINA_TRUE);
    elm_object_tree_focus_allow_set(surface_win, EINA_FALSE);

    Evas_Object *btn = contents(surface_win);

    evas_object_event_callback_add((surface_win), EVAS_CALLBACK_KEY_DOWN,
                                   key_down_cb, this);
    evas_object_event_callback_add((surface_win), EVAS_CALLBACK_KEY_UP,
                                   key_up_cb, this);
    // evas_event_callback_add((btn), EVAS_CALLBACK_KEY_DOWN, key_dows, this);
    LOG_ERROR("CJS surface_win %p %p", surface_win,
              evas_object_evas_get(surface_win));
    platformview_window_ = surface_win;
