      ...
      channel.SetMessageHandler(
          [api](const EncodableValue& message,
            ...
            try {
              ...
              const auto& encodable_force_use_hw_decoder_arg = args.at(0);
              if (encodable_force_use_hw_decoder_arg.IsNull()) {
                reply(WrapError("force_use_hw_decoder_arg unexpectedly null."));
                return;
              }
              const auto& force_use_hw_decoder_arg =
                  std::get<bool>(encodable_force_use_hw_decoder_arg);
              std::optional<FlutterError> output =
                  api->Initialize(force_use_hw_decoder_arg);
              if (output.has_value()) {
