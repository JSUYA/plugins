...
/*extern "C" size_t LWE_EXPORT createTizenViewInstance(
...
TizenView::TizenView(flutter::PluginRegistrar* registrar, int view_id,
    ...
    : PlatformView(registrar, view_id, nullptr),
      ...
      width_(width),
      height_(height) {
  use_sw_backend_ = true;  // IsRunningOnEmulator();
  if (use_sw_backend_) {
  ...
  texture_variant_ =
      std::make_unique<flutter::TextureVariant>(flutter::GpuSurfaceTexture(
          ...
          [this](size_t width,
            ...
            LOG_ERROR("CJS ObtainDescriptor %d %d", width, height);
  ...
  channel_->SetMethodCallHandler(
      [tizenview = this](const auto& call, auto result) {
        tizenview->HandleMethodCall(call, std::move(result));
      });
}

TizenView::~TizenView() { Dispose(); }

std::string TizenView::GetChannelName() {
  return "plugins.flutter.io/tizenview_" + std::to_string(GetViewId());
}

void TizenView::Dispose() {
...
void TizenView::Resize(double width, double height) {
  width_ = width;
  height_ = height;
...
void TizenView::Touch(int type, int button, double x, double y, double dx,
                      double dy) {
  LOG_ERROR("CJS: %f %f %d", x, y, type);
...
bool TizenView::SendKey(const char* key, const char* string,
...
void TizenView::SetDirection(int direction) {
  // TODO: Implement if necessary.
}

void TizenView::InitTizenView() {
  /*if (webview_instance_) {
    webview_instance_->Destroy();
    webview_instance_ = nullptr;
  }*/
  ...
  tbm_pool_->Prepare(315, 505);
  rendered_surface_ = candidate_surface_ = tbm_pool_->GetAvailableBuffer();
  LOG_ERROR("CJS Texture ID: %d", GetTextureId());
...
void TizenView::HandleMethodCall(
    const flutter::MethodCall<flutter::EncodableValue>& method_call,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
  tbm_pool_->GetAvailableBuffer();
  LOG_ERROR("CJS Texture ID: %d", GetTextureId());
  texture_registrar_->MarkTextureFrameAvailable(GetTextureId());
  ...
  /*
    if (method_name == "loadUrl") {
      ...
      if (GetValueFromEncodableMap(arguments, "baseUrl", &base_url)) {
        LOG_WARN("loadHtmlString: baseUrl is not supported and will be
    ignored.");
      }
...
FlutterDesktopGpuSurfaceDescriptor* TizenView::ObtainGpuSurface(size_t width,
                                                                size_t height) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!candidate_surface_) {
    if (rendered_surface_) {
      LOG_ERROR("CJS Return GpuSurface %d %d", width, height);
      return rendered_surface_->GpuSurface();
    }
    LOG_ERROR("CJS Return NULL %d %d", width, height);
    return nullptr;
  }
  if (rendered_surface_ && rendered_surface_->IsUsed()) {
    LOG_ERROR("CJS Release %d %d", width, height);
    tbm_pool_->Release(rendered_surface_);
  }
  rendered_surface_ = candidate_surface_;
  candidate_surface_ = nullptr;
  LOG_ERROR("CJS Return rendered %d %d", width, height);
  return rendered_surface_->GpuSurface();
}
