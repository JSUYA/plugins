...
TizenViewFactory::TizenViewFactory(flutter::PluginRegistrar* registrar)
...
PlatformView* TizenViewFactory::Create(int view_id, double width, double height,
                                       const ByteMessage& params) {
  return new TizenView(GetPluginRegistrar(), view_id, texture_registrar_, width,
                       height, *GetCodec().DecodeMessage(params));
}

void TizenViewFactory::Dispose() {  // LWE::LWE::Finalize();
}
