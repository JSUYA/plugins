...
void BufferUnit::Reset(int32_t width, int32_t height) {
  ...
  uint32_t* buffer_ = reinterpret_cast<uint32_t*>(surface_info.planes[0].ptr);
  LOG_ERROR("CJS Reset Buffer %d %d ", width, height);

  for (int w = 0; w < width; w++) {
    for (int h = 0; h < height; h++) {
      buffer_[h * width + w] = 0x55555555;
    }
  }
  ...
  gpu_surface_->release_callback = [](void* release_context) {
    ...
    buffer->UnmarkInUse();
    LOG_ERROR("CJS release_callback");
  };
...
BufferUnit* BufferPool::GetAvailableBuffer() {
  ...
  for (size_t index = 0; index < pool_.size(); index++) {
    ...
    if (buffer->MarkInUse()) {
      last_index_ = current;
      LOG_ERROR("CJS  GetAvailableBuffer");
      tbm_surface_info_s surface_info;
      tbm_surface_map(buffer->Surface(), TBM_SURF_OPTION_WRITE, &surface_info);
      uint32_t* buffer_ =
          reinterpret_cast<uint32_t*>(surface_info.planes[0].ptr);
      LOG_ERROR("CJS  Buffer");

      for (int w = 0; w < buffer->width_; w++) {
        for (int h = 0; h < buffer->height_; h++) {
          buffer_[h * buffer->width_ + w] = 0xAAAAAAAA;
        }
      }
      return buffer;
    }
...
BufferUnit* SingleBufferPool::GetAvailableBuffer() {
  ...
  tbm_surface_map(buffer->Surface(), TBM_SURF_OPTION_WRITE, &surface_info);
  uint32_t* buffer_ = reinterpret_cast<uint32_t*>(surface_info.planes[0].ptr);
  LOG_ERROR("CJS  Buffer %d %d", buffer->width_, buffer->height_);

  for (int w = 0; w < buffer->width_; w++) {
    for (int h = 0; h < buffer->height_; h++) {
      buffer_[h * buffer->width_ + w] = 0x55555555;
    }
  }
